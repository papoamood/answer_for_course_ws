...
# в колбэках.
# Воспринимайте очередь как траспорт для общения между тредами.
queue = Queue()
...
# Потом мы запустим тред методом WebSocketThread.start().
# Внутри вновь созданного треда, когда в нем запустится функция run_forever() -
# установится соединение с binance и внутри уже этой функции run_forever запустится свой event loop -
# цикл обработки сообщений, всякая внутренняя вебсокет-машинерия и вызовы
# функций-колбеков on_message, on_error и тд
#
...
def on_message(_wsapp, message):
    # К слову библиотека websocket-client ожидает именно такие сигнатуры функций, как
...
if __name__ == "__main__":
    ...
    while True:
        # И здесь мы все еще в нашем основном MainThread.

        # Получаем из очереди dict
        # Если очередь пуста - MainThread здесь ждет (блокируется)
    ...
    # завершится. Опять же просто для примера.
