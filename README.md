...
1. Только один тред может испольнять код в один момент времени. Даже на нескольких ядрах. Т.е. истинный параллелизм используя Thread python невозможен, потому-что GIL (Global Interpreter Lock).
...
`threading.Lock()` - это обьект, с помощью которого мы как-бы защищаем общую область памяти. Мы создаем один экзэмпляр `lock = threading.Lock()`, и все треды далее используют этот lock.
